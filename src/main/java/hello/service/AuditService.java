...
public class AuditService {
    ...
    public void insertAuditData(Integer managerId) {
        ...
        try {
            auditRepository.executeInsertAuditData(createTempTableSQL, insertSQL, managerId);
        } catch (Exception e) {
    ...
    public List<AuditDataDTO> selectAuditData() {
        ...
        String insertSql = "INSERT INTO #audit_data (audit_id, user_id, action, timestamp, details) VALUES (?, ?, ?, ?, ?)";
        List<AuditDataDTO> auditDataList = auditRepository.findAuditDataByManagerIdAndType(managerId, "SALES");
        for (AuditDataDTO auditData : auditDataList) {
            jdbcTemplate.update(insertSql, auditData.getId(), auditData.getUserId(), auditData.getAction(), auditData.getCreatedAt(), auditData.getDetails());
        }
